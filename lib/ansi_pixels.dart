...
class AnsiPixels {
  ...
  AnsiPixels(int width, int height) : _pixels = new List.generate(
          height, (_) => new List.filled(width, null), growable: false);
  factory AnsiPixels.generate(
      int width, int height, int generateCode(int x, int y)) {
  ...
  factory AnsiPixels.fromJson(List<List<int>> j) => new AnsiPixels.generate(
      j.first.length, j.length, (int x, int y) {
    try {
      final row = j[y];
      return row[x];
    } on RangeError {
  ...
  void forEach(void f(int x, int y, int code)) => _matrixIntForEach(_pixels, f);
